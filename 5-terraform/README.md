...
Welcome to **Phase 5** of the DevOps Masterclass!

You have a containerized app, observability tools, Kubernetes manifests, and a CI/CD pipeline. The final step is provisioning the actual cloud resources on AWS to host it all securely.
...
architecture-beta
    ...
    service params(database)[Parameter Store] in aws
    service iam(server)[IAM IRSA Roles] in aws
    service eks(server)[EKS Cluster] in aws
...
│   ├── terraform-secrets-manager.ps1
│   └── terraform-secrets-manager.sh
├── core/                     # Core AWS Infrastructure
...
```yaml
# In 3-kubernetes/helm/templates/external-secrets.yaml
...
- **Backend**: [1-1-apps/backend/README.md](../1-1-apps/backend/README.md)
